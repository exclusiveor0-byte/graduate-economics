...
## 3. 동적 시각화 1: 승수의 이중성 — '벽의 반작용력'과 '가치함수 접선 기울기(그림자 가격)'

아래 시뮬레이션은 자원 한도 $b$가 변화함에 따라 최적해 $x^*(b)$가 바인딩 경계해에서 자유 내부해로 전이되는 과정과, 이때 **라그랑주 승수 $\mu^*$가 어떻게 '선택 공간의 수직 항력'에서 '자원 공간의 가치함수 접선 기울기(그림자 가격)'로 1:1 대응되는지**를 보여준다.

- **목적함수**: 엄격오목 2차 효용 $f(x_1, x_2) = -(x_1 - 4)^2 - (x_2 - 4)^2 + 32$ (자유 극대점 $x^0 = (4, 4)$, $f(x^0) = 32$).
- **자원제약**: $x_1 + x_2 \le b$ ($b$를 $3.4$에서 $10.2$까지 확장).

![승수의 이중성: 벽의 반작용력(기하) vs 가치함수 접선 기울기(그림자 가격)](../assets/gif/kkt-multiplier-shadow-price.gif){.supplement-graphic fig-alt="선택 공간에서의 힘의 평형과 자원 공간에서의 가치함수 접선 기울기가 정확히 일치함을 증명하는 동적 시뮬레이션"}

### 도표 독해 가이드

#### 1. 좌측 패널: 선택 공간 $(x_1, x_2)$에서의 힘의 평형 (역학적 해석)
- **목적함수 견인력 $\nabla f(x^*)$ (청색 벡터)**:
  - 등고선의 수직 방향이자 더 높은 효용을 향해 자유 극대점 $x^0=(4, 4)$ 쪽으로 나아가려는 추진력이다.
- **제약벽의 수직 반작용력 $-\mu^* \nabla g(x^*)$ (적색 벡터)**:
  - 실행가능영역 경계선 $x_1 + x_2 = b$에서 바깥으로 탈출하지 못하도록 수직으로 가로막는 벽의 항력이다.
- **힘의 평형과 자원 완화 동학**:
  - $b < 8.0$ (자원 희소 구간): 최적해는 $x^*(b) = (b/2, b/2)$의 경계에 걸린다. 이때 $\nabla f = (8-b, 8-b)$와 벽의 반작용력 $\mu^* (1, 1)$이 정면으로 충돌하여 $\nabla f(x^*) = \mu^* \nabla g(x^*)$의 평형을 이룬다.
  - $b$가 커질수록 제약벽이 $x^0$에 가까워지며 밖으로 나가려는 압력이 줄어들어 **적색 화살표의 크기 $\mu^* = 8 - b$가 점차 축소**된다.
  - $b = 8.0$ (임계점): 제약벽이 $x^0=(4, 4)$를 지나는 순간, $\nabla f = 0$이 되고 벽의 반작용력도 $\mu^* \to 0$으로 완전히 소멸한다.
  - $b > 8.0$ (자원 잉여 구간): $x^0$가 실행가능영역 내부로 깊숙이 들어온다. 제약벽은 멀리 떨어져 있으므로 벽이 밀어내는 힘은 $0$($\mu^* = 0$)이다.

#### 2. 우측 상단 패널: 가치함수 $v(b)$와 접선 기울기 (포락선 정리, Envelope Theorem)
- 가치함수 곡선은 $b < 8$에서 포물선 형태 $v(b) = -\frac{1}{2}(b-8)^2 + 32$로 가파르게 상승하다가, $b \ge 8$에서는 천장 $v(b) = 32$에 도달하여 수평을 유지한다.
- 현재의 자원량 $b$에서 그은 **접선의 기울기(붉은 점선)**를 보면:
  $$
  \frac{dv(b)}{db} = \begin{cases} 8 - b, & b < 8, \\ 0, & b \ge 8. \end{cases}
  $$
  이 접선의 기울기가 **좌측 패널에서 벽이 밀어내는 힘의 크기 $\mu^*$와 소수점 둘째 자리까지 완벽히 일치**한다.
- **경제학적 의미**: 자원 $b$를 1단위 더 확보했을 때 얻을 수 있는 추가 효용의 크기, 즉 **자원의 그림자 가격(Shadow Price)**이 곧 라그랑주 승수이다.

#### 3. 우측 하단 패널: 상보여유성 전이 계기판 ($\mu^*(b)$ 곡선)
- 자원이 부족한 붉은 영역에서는 자원의 가치가 양수($\mu^* > 0$)이나, 자원이 남아도는 청록색 영역에서는 자원을 공짜로 더 줘도 목적함수를 높일 수 없으므로 잠재가격이 $0$($\mu^* = 0$)으로 떨어진다.

---

## 4. 동적 시각화 2: 코너해에서의 승수 분해와 상보여유성(Complementary Slackness) 동학

두 개 이상의 제약이 동시에 만나는 **코너해(Corner Solution / Vertex)**에서는 승수가 어떻게 작동하는가? 아래 시뮬레이션은 두 자원제약이 교차하는 꼭짓점에서 목적함수의 선호 방향(경사각 $\theta$)이 회전할 때, **$\nabla f$가 두 제약의 법선벡터가 형성하는 법선 원뿔(Normal Cone) 내부에서 어떻게 평행사변형 법칙으로 분해되는지**를 규명한다.

- **제약 1**: $x_1 + 2x_2 \le 6$ (법선벡터 $\mathbf{n}_1 = (1, 2)^\top$, 경사각 $63.4^\circ$).
- **제약 2**: $2x_1 + x_2 \le 6$ (법선벡터 $\mathbf{n}_2 = (2, 1)^\top$, 경사각 $26.6^\circ$).
- **교차 꼭짓점**: $x_c = (2, 2)$.

![코너해에서의 승수 분해와 법선 원뿔 및 상보여유성 동학](../assets/gif/kkt-corner-slackness-cone.gif){.supplement-graphic fig-alt="코너해에서 목적함수 그래디언트가 법선 원뿔 내부를 통과할 때 두 승수로 평행사변형 분해되는 과정과 실시간 상보여유성 막대그래프 동학"}

### 도표 독해 가이드

#### 1. 좌측 패널: 법선 원뿔(Normal Cone)과 평행사변형 분해
- **황금색 부채꼴 (Normal Cone)**:
  - 꼭짓점 $(2, 2)$에서 두 활성 제약의 외향 법선벡터 $\mathbf{n}_1$과 $\mathbf{n}_2$가 사잇각 $[26.6^\circ, 63.4^\circ]$ 내에서 생성하는 비음 결합 원뿔이다:
    $$
    N_{\mathcal{F}}(x_c) = \{ \mu_1 \mathbf{n}_1 + \mu_2 \mathbf{n}_2 \mid \mu_1 \ge 0, \; \mu_2 \ge 0 \}.
    $$
- **세 가지 국면의 동적 전이**:
  1. **$\theta < 26.6^\circ$ (제약 2만 바인딩)**: 목적함수 그래디언트가 완만하여 제약 2의 경계면이 최적이 된다. 제약 1은 여유가 있으므로 $\mu_1^* = 0$, $\mu_2^* > 0$이다.
  2. **$26.6^\circ \le \theta \le 63.4^\circ$ (코너해 진입)**: $\nabla f$가 황금색 법선 원뿔 내부로 진입한다!
     - KKT 정류성 $\nabla f = \mu_1^* \mathbf{n}_1 + \mu_2^* \mathbf{n}_2$에 따라, **청색 벡터 $\nabla f$는 적색 성분($\mu_1^* \mathbf{n}_1$)과 청록색 성분($\mu_2^* \mathbf{n}_2$)의 평행사변형 대각선으로 정확히 분해**된다.
     - $\theta = 45^\circ$일 때 두 자원의 병목 강도가 대칭을 이루며($\mu_1^* = \mu_2^*$), $\theta$가 한쪽으로 기울수록 해당 제약의 승수가 커진다.
  3. **$\theta > 63.4^\circ$ (제약 1만 바인딩)**: 목적함수 그래디언트가 가팔라져 제약 1의 경계면만 최적이 된다. 제약 2는 슬랙이 되어 $\mu_1^* > 0$, $\mu_2^* = 0$이 된다.

#### 2. 우측 패널: 실시간 상보여유성 대시보드
- 상단의 **KKT 승수 막대($\mu_1^*, \mu_2^*$)**와 하단의 **제약 여유도 막대($s_1, s_2$)**를 비교해 보라.
- 제약 1의 여유도 $s_1 > 0$인 순간에는 반드시 $\mu_1^* = 0$이며, 반대로 $\mu_1^* > 0$이 되는 순간 여유도는 칼같이 $0$으로 고정된다.
- 즉, **$\mu_j^* \cdot s_j = 0$이라는 상보여유성의 수리적 원리가 단 하나의 프레임 오차도 없이 엄밀하게 작동함**을 확인할 수 있다.
