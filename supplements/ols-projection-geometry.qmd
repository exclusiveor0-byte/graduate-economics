...
5. **오른쪽 패널 (Observation Space $\mathbb{R}^3$)**:
   3차원 관측공간에서 설명변수의 열공간 $\mathcal{C}(X) = \mathrm{span}\{\mathbf{1}, x\}$는 원점을 지나는 2차원 초록색 평면을 형성합니다. 종속변수 벡터 $y = (2, 1, 4)'$는 평면 밖의 점이며, OLS는 $y$에서 평면으로 수선의 발을 내린 점 $\hat{y}$를 찾습니다. 잔차 벡터 $\hat{e} = y - \hat{y}$는 평면 $\mathcal{C}(X)$와 정확히 직교($90^\circ$)합니다.
