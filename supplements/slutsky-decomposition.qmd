...
\hline
\textbf{변수} & \textbf{초기 균형 } (A) & \textbf{Hicks 보상 } (B_H) & \textbf{Slutsky 보상 } (B_S) & \textbf{최종 실제 균형 } (C) \\
\hline
\text{가격 } p_1 & 1.25 & 0.625 & 0.625 & 0.625 \\
\text{소득 } m & 100.0 & m^H \approx 70.71 & m^S = 75.0 & 100.0 \\
\text{1재화 소비 } x_1 & 40.0 & 56.57 & 60.0 & 80.0 \\
\text{2재화 소비 } x_2 & 50.0 & 35.36 & 37.5 & 50.0 \\
\text{효용 } u & u^0 \approx 44.72 & u^0 \approx 44.72 & u^S \approx 47.43 & u^1 \approx 63.25 \\
\hline
\text{대체효과 } (\Delta x_1) & - & \mathbf{+16.57} & \mathbf{+20.00} & - \\
\text{소득효과 } (\Delta x_1) & - & \mathbf{+23.43} & \mathbf{+20.00} & - \\
\text{총가격효과 } (\Delta x_1) & - & \multicolumn{3}{c}{\mathbf{+40.00} \; (= 80.0 - 40.0)} \\
\hline
\end{array}
$$

#### 시나리오 2: 가격 상승 ($p_1^0 = 1.0 \to p_1^1 = 1.5$) — 대화형 도구 기본값
$$
\begin{array}{l|c|c|c|c}
\hline
\textbf{변수} & \textbf{초기 균형 } (A) & \textbf{Hicks 보상 } (B_H) & \textbf{Slutsky 보상 } (B_S) & \textbf{최종 실제 균형 } (C) \\
\hline
\text{가격 } p_1 & 1.0 & 1.5 & 1.5 & 1.5 \\
\text{소득 } m & 100.0 & m^H \approx 122.47 & m^S = 125.0 & 100.0 \\
\text{1재화 소비 } x_1 & 50.0 & 40.82 & 41.67 & 33.33 \\
\text{2재화 소비 } x_2 & 50.0 & 61.24 & 62.50 & 50.00 \\
\text{효용 } u & 50.0 & 50.0 & u^S \approx 51.03 & u^1 \approx 40.82 \\
\hline
\text{대체효과 } (\Delta x_1) & - & \mathbf{-9.18} & \mathbf{-8.33} & - \\
\text{소득효과 } (\Delta x_1) & - & \mathbf{-7.49} & \mathbf{-8.33} & - \\
\text{총가격효과 } (\Delta x_1) & - & \multicolumn{3}{c}{\mathbf{-16.67} \; (= 33.33 - 50.0)} \\
\hline
\end{array}
$$
...
아래 인터랙티브 시뮬레이터는 슬라이더를 통해 새 가격 $p_1$, 소득 $m$, 선호 모수 $\alpha$를 자유롭게 조절하면서 Slutsky 보상과 Hicks 보상의 차이를 실시간으로 비교 관찰할 수 있는 도구입니다.
...
- **보상 규칙 라디오 버튼**:
  - **Slutsky 선택 시**: 청록색 점선 보상선이 검은색 초기 선택점 $x^0$를 정확히 관통함을 확인하십시오.
  - **Hicks 선택 시**: 청록색 보상점이 금색 초기 무차별곡선 $u^0$ 위에 엄밀히 안착함을 확인하십시오.
- **새 $p_1$ 슬라이더**: 가격을 높이거나 낮출 때 대체효과와 소득효과의 부호가 일치하는 정상재의 전형적인 수요 반응을 관찰하십시오.
- **하단 요약 패널**: 보상소득($m^c$)과 대체효과/소득효과의 정량적 수치가 자동으로 갱신됩니다.

---

## 10. 정적 그래픽 및 재현 환경

상호작용 자바스크립트를 지원하지 않는 인쇄물 및 모바일 환경을 위한 정적 다이어그램 벤치마크입니다.
...
- 모수 메타데이터: [`assets/data/slutsky-decomposition-metadata.json`](../assets/data/slutsky-decomposition-metadata.json)
- 파이썬 동학 애니메이션 생성기: [`scripts/generate_slutsky_gifs.py`](../scripts/generate_slutsky_gifs.py)
- 웹 대화형 소스코드: [`supplements/interactive/slutsky-decomposition/index.html`](interactive/slutsky-decomposition/index.html)

파이썬 애니메이션 GIF 2종은 프로젝트 루트에서 아래 명령어로 재생성할 수 있습니다:
